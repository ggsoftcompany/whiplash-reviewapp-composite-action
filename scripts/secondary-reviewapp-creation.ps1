...
Write-Output "GET Request URL: $uri"
$reviewAppInstance = $appReviewList.where{$_.branch.trim() -eq $targetBranchName}
if($reviewAppInstance.count -gt 0){
    Write-Warning "The pipeline: $herokuPipelineName already have a Review APP based on the branch: $targetBranchName."
    Write-Output $reviewAppInstance
    Write-Output "Finishing execution of the secondary review app process...."
    Write-Output " DONE "
    return
}
